...
```bash
task$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
task$ sam local start-api
task$ curl http://localhost:3000/
```
...
```bash
task$ sam logs -n HelloWorldFunction --stack-name task --tail
```
